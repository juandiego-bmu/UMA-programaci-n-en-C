...
void imprimir_suma_umbral(double umbral, struct Datos * d1){
    ...
    for(int i = 0; i<d1->nelms; i++){
        ...
        if(d1->elm[i]>=umbral){
            ...
            suma+=*(d1->elm+i); //quiero acceder al valor así q tengo q poner *
        }
...
double calcular_media(struct Datos * d1){
    ...
    for(int i= 0; i<d1->nelms; i++){
        ...
        media+=*(d1->elm+i); //quiero acceder al valor así q tengo q poner *
...
void leer_elementos(struct Datos * d1){
    ...
    for(int i = 0; i<d1->nelms; i++){
        ...
        scanf("%d", (d1->elm+i));   //para leer no pongo *, pq quiero acceder a la posición no al valor
